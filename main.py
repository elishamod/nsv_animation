...
class DualEllipticOrbits(ThreeDScene):
    def construct(self):
        ...
        a2, b2 = 2.5, 1.5  # ellipse 1 parameters
        ball_radius = 0.2
        ...
        def ellipse1_func(t):
            theta = 2 * PI * t
            return np.array([a1 * np.cos(theta) + c1, b1 * np.sin(theta), 0])
        ...
        def ellipse2_func(t):
            theta = 2 * PI * t
            return np.array([0, a2 * np.cos(theta) + c2, b2 * np.sin(theta)])

        ellipse2 = ParametricFunction(ellipse2_func, t_range=[0, 1], color=BLUE)

        # Fast-rendering spheres: low resolution, solid color
        ball1 = Sphere(radius=ball_radius, resolution=(12, 24)).set_color(RED)
        ball2 = Sphere(radius=ball_radius, resolution=(12, 24)).set_color(RED)
